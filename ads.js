...
import { html, Page, BRAND, todayStr } from "./ui.js?v=__V__";
import { useLoader, fnUrl, ENV } from "./db.js?v=__V__";
...
function Ad({ f, v }) {
  ...
  const dots = (size) => html`<span style="display:inline-flex;vertical-align:middle">${dot(size, BRAND.rose)}${dot(size, BRAND.teal, true)}</span>`;

  if (strip) {
    return html`<div style=${`width:${px(w)};height:${px(h)};background:${BRAND.paper};border:2px solid ${BRAND.ink};display:flex;align-items:center;gap:${px(m * 0.18)};padding:0 ${px(m * 0.3)};box-sizing:border-box;overflow:hidden`}>
      ${dots(m * 0.42)}
      ...
      <span style=${`background:${BRAND.rose};color:#fff;font:700 ${px(m * 0.24)}/1 Inter;padding:${px(m * 0.16)} ${px(m * 0.3)};border-radius:999px;white-space:nowrap`}>${v.cta}</span>
    </div>`;
  }
  if (banner) {
    return html`<div style=${`width:${px(w)};height:${px(h)};background:${BRAND.paper};border:2px solid ${BRAND.ink};display:flex;flex-direction:column;align-items:center;justify-content:center;gap:${px(h * 0.05)};padding:${px(h * 0.06)};box-sizing:border-box;text-align:center;overflow:hidden`}>
      ...
      <div style=${`background:${BRAND.rose};color:#fff;font:700 ${px(h * 0.075)}/1 Inter;padding:${px(h * 0.045)} ${px(h * 0.1)};border-radius:999px`}>${v.cta}</div>
    </div>`;
  }
  // posters: square / portrait / story / landscape / og / x
  const tall = h / w > 1.4;
  return html`<div style=${`width:${px(w)};height:${px(h)};background:${BRAND.paper};box-sizing:border-box;padding:${px(m * 0.07)};display:flex;overflow:hidden`}>
    <div style=${`flex:1;border:${px(Math.max(2, m * 0.004))} solid ${BRAND.ink};display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:${px(m * 0.06)};gap:${px(m * (tall ? 0.035 : 0.028))};position:relative`}>
      <div style=${`font:600 ${px(m * 0.021)}/1 Inter;letter-spacing:.3em;color:${BRAND.muted};text-transform:uppercase`}>est. 2026 · find your people</div>
      <div>${dots(m * 0.055)}</div>
      <div style=${`font:500 ${px(m * 0.085)}/1 'Cormorant Garamond',serif`}>collide</div>
      <div style=${`border-top:${px(Math.max(2, m * 0.005))} double ${BRAND.ink};width:55%`}></div>
      <div style=${`font:400 ${px(m * (v.quote ? 0.075 : 0.082))}/1.2 Lacquer,cursive;max-width:92%`}>${v.head}</div>
      <div style=${`font:italic 500 ${px(m * 0.036)}/1.4 'Cormorant Garamond',serif;color:${BRAND.ink2};max-width:80%`}>${v.sub}</div>
      <div style=${`background:${BRAND.rose};color:#fff;font:700 ${px(m * 0.036)}/1 Inter;padding:${px(m * 0.026)} ${px(m * 0.06)};border-radius:999px;margin-top:${px(m * 0.01)}`}>${v.cta}</div>
      <div style=${`position:absolute;bottom:${px(m * 0.035)};font:600 ${px(m * 0.02)}/1 Inter;letter-spacing:.18em;color:${BRAND.faint};text-transform:uppercase`}>collide.city</div>
    </div>
  </div>`;
}
...
export function AdsPage({ client, flash }) {
  ...
  const [evPick, setEvPick] = useState("");
  const { data: eventRows } = useLoader(() => client.from("activities").select("id,title,date").eq("visibility", "public")
      .or("date.gte." + todayStr() + ",and(date.is.null,expires_at.gt." + new Date().toISOString() + ")")
      .order("date", { ascending: true, nullsFirst: false }).limit(50), [client], { flash, where: "Ads" });
  const events = eventRows || [];
  const evId = evPick || events[0]?.id || "";
  const setEvId = setEvPick;
  ...
  const base = dest === "event" ? fnUrl("share", { e: evId }) : dest;
  ...
  return html`<${Page} title="Ads" sub="funnel creative — pick a format, download the asset, copy the link">
    ...
    <div class="ads-grid">
      <div>
        ...
        <div class="section-label u-mt-2">Message</div>
        <div class="chips">
          ${VARIANTS.map((x) => html`<button class="chip" aria-pressed=${vkey === x.key ? "true" : "false"} onClick=${() => setVkey(x.key)}>${x.label}</button>`)}
        </div>

        <div class="section-label u-mt-2">Deeplink</div>
        ...
        <div class="section-label u-mt-2">Brand asset kit</div>
        <div class="inset" style="padding:8px 14px">
          ${ASSETS.map((a) => html`<div class="ads-asset">
            <span>${a.name}</span>
            <span class="u-row" style="gap:6px">
              <a class="btn sm ghost" href=${a.url} target="_blank" rel="noopener">open</a>
              <button class="btn sm ghost" onClick=${() => copy(a.url, "Asset URL copied")}>copy url</button>
            </span>
          </div>`)}
        </div>
      </div>
